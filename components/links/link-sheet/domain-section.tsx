...
export default function DomainSection({
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      ...
      setModalOpen(true);
      setData({ ...data, domain: `${process.env.DEFAULT_DOMAIN || "papermark.io"}`});
      return;
    }
  ...
  return (
    <>
      ...
      <div className="flex">
        <Select
          defaultValue={data.domain || `${process.env.DEFAULT_DOMAIN || "papermark.io"}`}
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== `${process.env.DEFAULT_DOMAIN || "papermark.io"}`
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value={`${process.env.DEFAULT_DOMAIN || "papermark.io"}`} className="hover:bg-muted">
              papermark.io
            </SelectItem>
        ...
        {data.domain && data.domain !== `${process.env.DEFAULT_DOMAIN || "papermark.io"}` ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== `${process.env.DEFAULT_DOMAIN || "papermark.io"}` ? "flex" : "",
            )}
            placeholder="deck"
            onChange={(e) => {
      ...
      {data.domain && data.domain !== `${process.env.DEFAULT_DOMAIN || "papermark.io"}` && !isDomainVerified ? (
